...
| File | Action | Notes |
|------|--------|-------|
| `.chezmoi.toml.tmpl` | **New** | Init-time template: 3 `promptStringOnce` under `[data.git]` + folded static config (see REQ-11) |
| `.chezmoidata.toml` | Modified | Drop `user_name`, `user_email`, `signingkey`; keep `default_branch`, `[brew]`, `[editor]`, `[os]` |
| `dot_config/chezmoi/chezmoi.toml` | **Deleted** | Folded into `.chezmoi.toml.tmpl` per REQ-11 |
...
### REQ-11: Consolidate `~/.config/chezmoi/chezmoi.toml` ownership in `.chezmoi.toml.tmpl` (design revision)

The system MUST fold the contents of the source-managed file `dot_config/chezmoi/chezmoi.toml` into the new init-time template `.chezmoi.toml.tmpl`, and the source-managed file MUST be deleted from the source state. The rendered `~/.config/chezmoi/chezmoi.toml` MUST be the single writer for both the `[data.git]` section (via init-time prompts) and the static config sections (`[git]`, `[diff]`, and the Interpreters / age comments).

#### Scenario: No source-managed conflict on `chezmoi apply`

- GIVEN a fresh clone where `chezmoi init` has produced `~/.config/chezmoi/chezmoi.toml` with both `[data.git]` and static sections
- WHEN `chezmoi apply` runs (or any subsequent init)
- THEN the rendered `~/.config/chezmoi/chezmoi.toml` is preserved as-is
- AND no source-managed file overwrites the `[data.git]` section

#### Scenario: Static config is preserved across init and apply

- GIVEN a `chezmoi.toml.tmpl` that contains both `[data.git]` prompts and the static `[git] autoAdd/autoCommit/autoPush/commitMessageTemplate` and `[diff] pager` settings
- WHEN `chezmoi init` renders the template into `~/.config/chezmoi/chezmoi.toml`
- THEN the rendered file includes both the prompted data and the static config
- AND subsequent `chezmoi apply` does not remove the static config

#### Scenario: Loss of chezmoi.toml-level comments is documented

- GIVEN the current `dot_config/chezmoi/chezmoi.toml` contains comments about Interpreters.ps1 and age encryption setup
- WHEN the contents are folded into `.chezmoi.toml.tmpl`
- THEN the comments are preserved in the init-time template (the rendered config is TOML and cannot have comments, but the source-of-truth template retains them)
